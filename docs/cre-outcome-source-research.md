...
As of the audit date, the ignored local warehouse contains 693 observations for each of five institutional multifamily outcomes: effective rent, reported YoY rent growth, same-store apartment inventory, physical occupancy, and exactly derived vacancy. The active panel therefore contains 3,465 candidates across 27 source-defined markets and 30 quarters (2019-Q1 through 2026-Q2). A legacy-heading backfill added 262 occupancy and 262 vacancy observations from preserved SEC evidence. The same correction explicitly selects current-quarter rather than YTD occupancy in multi-period tables and rejects date/header rows by governed market-name shape. Apartment units are retained as `same_store_inventory`, not total metro inventory, and every filing passes an independent unit-count comparison between the rent and occupancy tables. The active vintage has complete source URLs, filing dates, units, sample counts, local snapshot hashes, no structural verification errors, no duplicate periods, and no unit or methodology inconsistencies.
