...
- Organization owns users, deals, documents, append-only document-purge events, extracted values, immutable semantic entities, manual assumptions, review decisions, reconciliation runs, findings, export artifacts and audit events.
...
- Original document bytes may be purged only by a reauthenticated administrator. The document becomes a provenance tombstone; the append-only purge event records actor, reason, original hash/size and time.
...
- Semantic entity stores a canonical repeated rent-roll, operating-account/period, lease-schedule or debt-term row with document/version/source-row provenance, a content hash and the exact constituent extracted-value IDs. Its review state is derived: approved only when every constituent cell is approved through the existing append-only decision chain.
