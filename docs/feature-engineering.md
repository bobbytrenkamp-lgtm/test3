...
The governed macro layer exposes period-end CPI and exact one-year CPI growth from the existing public Federal Reserve series. CPI is broadcast nationally only after deterministic period alignment; it is never treated as a locally observed county statistic. The first operating-expense research specification uses CPI growth and local personal-income growth, remains candidate-only, and must satisfy the same real-target, longitudinal-depth, walk-forward, holdout, and baseline gates as rent-growth models.
