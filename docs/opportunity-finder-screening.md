...
SQLite schema version 11 persists three core organization-scoped Finder records:
...
The current screening projection is derived from the latest immutable run. Operational integrity reproduces every screening result from its persisted evidence version and evaluation timestamp, verifies all hashes and membership bindings, and fails closed on tampering. Backup format 11.0 includes these records, the governed review bridge, and explicit promotion evidence; prior backup formats remain readable.
...
Schema version 11 includes immutable `opportunity_candidate_review_artifacts` and append-only `opportunity_candidate_review_decisions`. An analyst or administrator can send only the latest, currently screened evidence version to Opportunity Review. The server builds the artifact from persisted records; the client cannot author or replace its evidence. One artifact is allowed per immutable screening run.

The artifact binds the candidate, evidence version, screening run, screening-result hash, and explicit `NO_VALIDATED_OPPORTUNITY_SCORE` state. Operational integrity re-hashes the artifact and decision chain and verifies their relational bindings. Backup format 11.0 retains both tables.

Review itself does not create a deal, promote a candidate, calculate a return, apply an Underwrite assumption, or produce a Test2 handoff. Reviewers and administrators may independently approve or reject the evidence package. The artifact creator cannot approve it. Approval also fails if the candidate was archived, newer evidence exists, or a newer screening superseded the artifact. An insufficient-evidence screening cannot be approved, and changed evidence requires a new screening and artifact.

## Explicit Deal Pipeline promotion

An analyst or administrator may explicitly promote an active candidate only after the latest evidence has a current screening artifact whose latest independent decision is approved. The server re-verifies the artifact and decision hashes inside one write transaction. It then creates a new `not_processed` Deal Pipeline intake, records an immutable `opportunity_candidate_promotions` artifact, and transitions the candidate to `promoted_to_diligence`.

Promotion accepts no client-authored deal fields or underwriting values. It copies only candidate identity fields, creates no assumptions, and hard-codes `automatic_underwrite_apply = false`. Repeated promotion, stale evidence, superseding screening, missing approval, rejection, archived candidates, and integrity failure all fail closed.
