...
The page reports warehouse rows and periods, source/metric/geography coverage, feature-panel versions, property-type CRE target coverage, target readiness gates, installed model artifacts, cross-check status, and the number of validated real-data models. Empty states are deliberate. If no legitimate CRE targets exist, the page says so instead of displaying synthetic research as production evidence.
