...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if len(parts) == 4 and parts[:2] == ["api", "deals"] and parts[3] == "location-analysis":
                self._authorize_post(user, "assumption.create")
                length = int(self.headers.get("Content-Length", "0"))
                if length < 0 or length > 16 * 1024 * 1024:
            ...
            if len(parts) == 4 and parts[:2] == ["api", "assumption-runs"] and parts[3] == "handoff":
                self._authorize_post(user, "export.generate")
                return self._json(201, self.service.create_assumption_run_handoff(user["organization_id"], user["id"], parts[2]))
