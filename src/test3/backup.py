...
def create_backup(data_dir: Path, destination: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        manifest = {"format": "test3-backup/11.0", "schemaVersion": SCHEMA_VERSION, "createdAt": datetime.now(timezone.utc).isoformat(), "files": {name: {"sha256": _hash(path), "bytes": path.stat().st_size} for path, name in files}}
...
def verify_backup(archive_path: Path, max_expanded_bytes: int = 2 * 1024 * 1024 * 1024) -> dict:
    with tempfile.TemporaryDirectory() as temporary, zipfile.ZipFile(archive_path) as archive:
        ...
        if manifest.get("format") not in ("test3-backup/1.0", "test3-backup/2.0", "test3-backup/3.0", "test3-backup/4.0", "test3-backup/5.0", "test3-backup/6.0", "test3-backup/7.0", "test3-backup/8.0", "test3-backup/9.0", "test3-backup/10.0", "test3-backup/11.0"):
        ...
        operational = []
        if manifest["format"] in ("test3-backup/3.0", "test3-backup/4.0", "test3-backup/5.0", "test3-backup/6.0", "test3-backup/7.0", "test3-backup/8.0", "test3-backup/9.0", "test3-backup/10.0", "test3-backup/11.0"):
