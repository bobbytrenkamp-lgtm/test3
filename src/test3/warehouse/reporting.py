...
def coverage_report(paths) -> list[dict]:
    ...
    with duckdb.connect(":memory:") as db:
        result = db.execute(f"""SELECT source_id,source_dataset,metric,geography_type,period_type,
            min(observation_date) earliest,max(observation_date) latest,count(*) observations,
            count(DISTINCT geography_id) geographies FROM {source}
            GROUP BY ALL ORDER BY source_id,source_dataset,metric,geography_type,period_type""")
